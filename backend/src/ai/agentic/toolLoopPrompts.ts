...
export function buildToolLoopInstructions(input: AgentRunInput, memoryBlock?: string): string {
  ...
  const parts: string[] = [
    ...
    "TOOL SELECTION: prefer fast tools. web_researchTask answers most research/comparison questions in seconds with citations — use it (multiple calls for multiple subjects are fine). Reserve web_deepResearch for explicit deep-dive requests, never more than one call, and never in parallel with other deepResearch calls.",
    "Ground every factual claim in tool results and preserve source URLs.",
    "NEVER perform an external write directly. To send email or change CRM data, call the matching typed prepare-approval tool from your toolkit (hubspot_prepareCreateApproval, hubspot_prepareUpdateApproval, hubspot_prepareNoteApproval, hubspot_prepareTaskCreateApproval, gmail_prepareSendApproval, stripe_preparePaymentLinkApproval, …) to PROPOSE the action — a human approves it later. Do not use the generic approval_create when a typed prepare-approval tool exists for the action. If a tool returns status 'error' with a suggested tool, call that suggested tool next — do not give up or claim the tool is unavailable. If a tool returns status 'approval_required' or 'blocked', do not retry it; either propose via a prepare-approval tool or stop and summarize.",
    ...
    "STEP BUDGET: you have a limited number of steps. Never repeat a tool call that failed or returned empty with the same input — after two failures, change approach or stop and report what you found. If a CRM record is not found, do not keep searching: propose creating it via the matching prepare-approval tool (e.g. hubspot_prepareCreateApproval) or tell the user it does not exist. Always reserve your final step for the written answer.",
